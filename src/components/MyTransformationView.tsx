...
export default function MyTransformationView({ progress, days, lang, onBackToHome }: MyTransformationViewProps) {
  ...
  const getMoodEmojiAndLabel = (moodKey: string) => {
    switch (moodKey) {
      ...
      case 'neutral':
        return { emoji: '😐', label: trans.moodNeutral, color: 'from-slate-400 to-slate-500 bg-slate-500/20 text-slate-600 dark:text-ink-muted border-slate-500/20' };
      case 'heavy':
            ...
            >
              ...
              {progressInsight && (
                <div className="p-5 rounded-2xl bg-gradient-to-br from-accentgold/10 to-rosegold/5 border border-accentgold/20 space-y-2.5">
                  ...
                  <p className="text-sm text-slate-600 dark:text-ink-muted leading-relaxed font-sans font-semibold">
                ...
                <div className="relative border-l-2 border-rose-100/50 dark:border-rosegold/15 pl-6 ml-4 space-y-8 py-2 text-left">
                  {milestonesList.map((m, idx) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-500 dark:text-ink-muted leading-relaxed font-sans max-w-xl italic">
                        "{m.desc}"
                      </p>
                    </motion.div>
                  ))}
